...
1. **Login seguro** (`/` ou `/sign-in` + `/auth/callback` + `/dashboard`):
   ...
   - Quando a aba é reaberta com tokens persistidos, `/sign-in` usa `getSession` e `onAuthStateChange` para reconhecer a sessão e
     redirecionar automaticamente para o destino configurado, evitando loops infinitos em "Confirmando seu acesso...".
