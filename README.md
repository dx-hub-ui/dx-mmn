...
- `public.memberships.user_id` referencia diretamente `public.profiles(id)` com `ON DELETE CASCADE`, permitindo que o PostgREST exponha o relacionamento `profile:profiles` utilizado pelo cliente Supabase e garantindo que memberships órfãos sejam removidos quando um perfil espelhado for excluído.
