...
O componente de topo usa um `Avatar` do `@vibe/core` com `type = Avatar.types.ICON`, `size = Avatar.sizes.MEDIUM` e plano de fundo `royal`, sobreposto por um `Counter` negativo fixado no canto superior direito (`pointer-events: none`). Essa combinação garante contraste AA, segue os tokens de espaçamento do shell e evita regressões nas props de `IconButton` que não suportam badges customizados. O wrapper renderizado na topbar é diretamente o `NotificationsBell`, evitando ícones mudos: quando o feature flag `notifications_v1` está ativo, o mesmo botão abre o `Dialog` em formato popover; se o flag estiver desligado ou não houver organização ativa, mostramos o ícone desabilitado mantendo o alinhamento visual sem interceptar cliques.
