...
- O `DragOverlay` passou a replicar largura e altura do cartão original durante o arraste, garantindo que o cursor permaneça dentro do preview mesmo em colunas estreitas ou com conteúdo responsivo.
...
- **Modal de contato completo** (`ContactModal`): cabeçalho com estágio editável, atalhos (WhatsApp/ligar/e-mail), sub-header com origem/tags e tabs Atividades, Dados, Próximo passo e Indicações. A timeline consome `contact_events` com filtros contextuais e navegação com setas (`←/→`). Fechar com `Esc` retorna foco à linha; `O`/enter abre a partir do grid. O container principal agora depende dos estilos globais de `.ReactModal__Overlay`/`.ReactModal__Content` para herdar a geometria padrão dos modais.
