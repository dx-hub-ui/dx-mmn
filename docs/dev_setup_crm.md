...
> **Políticas RLS:** a função `can_access_membership` (security definer) desativa temporariamente o RLS enquanto consulta `visible_membership_ids`, permitindo que `memberships_select_visible` calcule visibilidade sem disparar o erro `42P17` de recursão infinita. A migração `008_fix_dashboard_memberships.sql` também renomeia o parâmetro para `target_membership_id`, evitando o erro `42702 column reference "membership_id" is ambiguous` ao consultar memberships pelo dashboard.
