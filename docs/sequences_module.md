...
- **Modal de passos** recebe `state` obrigatório e objeto de opções contendo `anchorStepId` e `position` dentro de `onSubmit`, garantindo que a tipagem reflita exatamente o contrato das server actions e evitando assinaturas inválidas durante o build.
