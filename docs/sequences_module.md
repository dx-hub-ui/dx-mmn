...
- **Criação imediata de etapas** elimina o modal ao clicar em “Adicionar”: o cartão surge diretamente na coluna esquerda com campo de título editável inline, enquanto o painel direito permanece focado na descrição curta para ajustes rápidos.
- **Limpeza do rascunho de título** remove o mapa auxiliar via `delete`, garantindo que a etapa recém-criada saia do estado de edição assim que o usuário conclui ou cancela a alteração, mantendo o build livre de avisos `no-unused-vars`.
- **Modal de passos** recebe `state` obrigatório e transmite posicionamento via `state.position` (`kind` + `referenceId`), alinhando o contrato das server actions sem duplicar parâmetros (`anchorStepId`, `position`) e mantendo o build limpo.
...
- **Persistência direta** garante que o textarea use um único `noteDraft` controlado e que o toggle de pausa escreva diretamente no passo selecionado, evitando estados locais duplicados que gerem avisos de lint.
...
- **Hidratação simplificada** no manager elimina o estado local `isHydrated`, já que o skeleton inicial cobre o período pré-render e evita referências indefinidas durante o build do Next.js.
- **Ordenação resiliente** converte métricas ausentes (`openRate`, `replyRate`, `clickRate`, inscrições) em `null` antes de comparar, permitindo que o typecheck aceite os cenários sem dados e mantendo o sort estável.
